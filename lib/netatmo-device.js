...
class NetatmoDevice {
	...
	refreshDeviceData(callback) {
		...
		// decide which cache to query, L1 or L2
		var append = "";
		if(this.l2cache == false) {
			this.l2cache = true;
			console.log("L2 on");
		} else {
			append = "_l2"
		}
		console.log("append: "+(this.deviceType)+append);

		this.cache.get((this.deviceType)+append, function(err, data) {
			if (!err) {

...
const REFRESH_CHECK_RATE = 5 * 60 * 1000; // in milliseconds, 5 minutes
const DEFAULT_CACHE_TTL = REFRESH_CHECK_RATE / 1000; // in seconds, use config["ttl"] to override
...
class NetatmoDevice {
	...
	refreshDeviceData(callback) {
		...
		this.cache.get((this.deviceType), function(err, data) {
			if (!err) {
				if (data === undefined || data === null || this.refreshCheckRunning) {

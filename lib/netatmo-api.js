...
let authentication_failed_error = null;
let filename;
...
netatmo.prototype.waitUntilAuthenticated = function (callback, onAuthenticated) {
  if (authentication_failed_error) {
    if (callback) {
      callback(authentication_failed_error);
    } else {
      this.emit('error', authentication_failed_error);
    }
    return true;
  }
  ...
  onAuthSuccess = () => {
    this.removeListener('authentication-failed', onAuthFailure);
    onAuthenticated.call(this);
  };

  this.once('authenticated', onAuthSuccess);
  this.once('authentication-failed', onAuthFailure);
...
netatmo.prototype.authenticate = function (args, callback) {
  ...
  axios.post(url, params, {
    ...
    authentication_failed_error = null;
    ...
    const error = this.handleRequestError(err, response, body, 'Authenticate error', true);
...
netatmo.prototype.authenticate_refresh = function () {
  ...
  axios.post(url, params, {
    ...
    const body = response ? response.data : null;
    if (access_token === null) {
      const error = this.handleRequestError(err, response, body, 'Authenticate refresh error', true);
      authentication_failed_error = error;
      this.emit('authentication-failed', error);
      return this;
    }
...
netatmo.prototype.getStationsData = function (options, callback) {
  ...
  if (this.waitUntilAuthenticated(callback, () => {
  ...
  axios.post(url, params, {
    ...
    const response = err.response;
    const body = response ? response.data : null;
    const error = this.handleRequestError(err, response, body, 'getStationsDataError error');
    if (callback) {
      callback(error);
    }
    return error;
...
netatmo.prototype.getHealthyHomeCoachData = function (options, callback) {
  ...
  axios.get(url).then((response) => {
    ...
    const response = err.response;
    const body = response ? response.data : null;
    const error = this.handleRequestError(err, response, body, 'getHealthyHomeCoachData error');
    if (callback) {
      callback(error);
    }
    return error;
